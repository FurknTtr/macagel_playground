...
const getAuthedUserId = function (req, res) {
  if (!req.userId) {
    createResponse(res, 401, { message: "Yetkilendirme gerekli" });
    return null;
  }
...
const addRating = async function (req, res) {
  try {
    ...
    const reviewerId = getAuthedUserId(req, res);
...
const updateRating = async function (req, res) {
  try {
    ...
    const existing = await Rating.findById(rateId);
    if (!existing) return createResponse(res, 404, { message: "Değerlendirme bulunamadı" });

    if (existing.reviewer.toString() !== reviewerId.toString()) {
      return createResponse(res, 403, { message: "Bu değerlendirmeyi güncelleme yetkiniz yok" });
    }

    if (rating && (rating < 1 || rating > 5)) {
      return createResponse(res, 400, { message: "Geçersiz puan" });
    }
    ...
    createResponse(res, 200, { message: "Değerlendirme güncellendi", rating: existing });
  } catch (error) {
...
const deleteRating = async function (req, res) {
  try {
    ...
    if (existing.reviewer.toString() !== reviewerId.toString()) {
      return createResponse(res, 403, { message: "Bu değerlendirmeyi silme yetkiniz yok" });
    }
